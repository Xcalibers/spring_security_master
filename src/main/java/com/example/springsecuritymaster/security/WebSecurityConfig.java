...
public class WebSecurityConfig {
    ...
    public UserDetailsService userDetailsService() {
        ...
        var william = User.withUsername("william")
                .password(("william"))
                .roles(DEPARTMENTS_CREATE, DEPARTMENTS_READ, DEPARTMENTS_PAGE_VIEW).build();

        var lucas = User.withUsername("lucas")
                .password(("lucas"))
                .roles(CUSTOMERS_READ, CUSTOMERS_PAGE_VIEW).build();
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .permitAll().and().logout().logoutUrl("/logout").logoutSuccessUrl("/login").permitAll();
        http.csrf().disable();
        return http.build();
    }
